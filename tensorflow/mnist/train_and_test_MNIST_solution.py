...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        # Build the graph
        cnn = CNN(patch_size=FLAGS.patch_size,
                  num_filters_fist_layer=FLAGS.num_filters_fist_layer,
        ...
        # Bring into shape expected by image_summary
        V = tf.reshape(V, (-1, 5, 5, 1))
        image_summary_op2 = tf.summary.image("kernel_layer2", V, 5)
        # Train Summaries
        train_summary_op = tf.summary.merge([loss_summary, acc_summary, image_summary_op, image_summary_op2, grad_summaries_merged])
